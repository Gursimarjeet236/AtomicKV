...
What started as a simple in-memory cache evolved into a distributed, fault-tolerant system capable of handling nearly 10,000 concurrent operations per second. 
...
## 1. The Core Engine: Solving C10k with Java NIO (`epoll`)
The standard approach to building a Java server is spawning a new `Thread` for every client. This works fine for 100 users, but crashes and burns at 10,000 users because the CPU spends more time context-switching between threads than actually processing data. 

AtomicKV abandons multithreading for its network layer. Instead, it uses **Java NIO (New I/O)** with non-blocking `SocketChannel`s. Under the hood on Linux systems, Java NIO's `Selector` acts as a wrapper around the Linux kernel's **`epoll` API**. A *single* main thread monitors thousands of sockets simultaneously. It only wakes up to process data when the OS kernel explicitly notifies it that a socket is ready for reading or writing, completely eliminating context-switching overhead.
...
For lightning-fast reads, data sits in a custom-built Least Recently Used (LRU) Cache. It uses a combination of a `HashMap` and a `LinkedList` to achieve **`O(1)` time complexity** for both lookups and evictions. It also implements Lazy Expiration (TTL)—expired keys are cleaned up upon access rather than wasting CPU cycles on a background thread. 
...
When the RAM fills up, data isn't lost. It falls back to a custom **Order-3 B-Tree** built entirely from scratch using Java's `RandomAccessFile`. 
...
sequenceDiagram
    participant Client
    participant NIO as Server - NIO Selector
    participant KV as KVStore
    participant LRU as RAM Cache
    participant Disk as B-Tree - Disk

    Client->>NIO: "SET my_key my_value\n"
    NIO->>KV: set("my_key", "my_value")
...
* **Read Repairs & Anti-Entropy:** Background worker threads continuously query and compare data between nodes to repair stale records using background network tasks.
...
* `src/AtomicKVServer.java`: Main NIO loop, background workers, and TCP handling.
...
The system was benchmarked using a custom Python script simulating concurrent clients to test the NIO architecture.
...
### AWS EC2
Tested on Ubuntu 22.04 LTS. Ensure your Security Group allows inbound TCP traffic on port 8081. Clone the repository, compile using `build.sh`, and run `java -cp out AtomicKVServer 8081`.
...
## What I Learned
This project was an exercise in understanding system-level programming. Implementing a B-Tree from scratch, working with Java NIO multiplexing, and dealing with distributed consensus and lock-free structures provided a much deeper appreciation for production databases.
